...
6. **Tuples are available** (`tuple<T1,T2>[a, b]`, EBNF `iunctaExpr`): multi-return helpers (e.g. `callable_boundary_return`'s `(ret, err)`) may return tuples directly; use named result classes where the shape is reused or needs field names (verify tuple codegen in the en build on first use).
