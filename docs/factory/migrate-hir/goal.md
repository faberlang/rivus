...
- **No tuples.** `Struct(DefId, Vec<(Symbol, HirExpression)>)` → a named pair `class` (e.g. field-entry `{name: Symbol, value: numerus}`); `HirLiteral` values and `(Symbol, HirExpression)` pairs in object entries get the same treatment. `HirObjectField { key, value }` already a named class.
