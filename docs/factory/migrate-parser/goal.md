...
- **No tuples.** `checkpoint() -> (pos, NodeId)` becomes `class Checkpoint { pos: numerus, next_node_id: numerus }` (proven pattern: `norma/src/json/cursor.fab` `Resultus`). `Result<T, ParseError>` return/error flow: parser already accumulates `errors` on `self`; port as an en outcome genus (`Resultus`-style) in `error.fab` carrying value-or-nihil plus having pushed any `ParseError` onto `parser.errors`, preserving the `?`→recovery structure.
