...
2. **Union variant names are module-flat — per module, not per package.** Unlike Rust enums, Faber union variants are NOT namespaced by their union: within one `.fab` module, all variant names share one namespace (`InvalidAnnotation` (parse ∩ semantic), `InvalidAssignmentTarget` (parse ∩ semantic), `Warning` (severity ∩ semantic), `Lex`/`Parse` (phase ∩ phaseError) collided — `SEM005:duplicate_definition`). Renamed to `SemanticInvalidAnnotation`, `SemanticInvalidAssignmentTarget`, `SemanticWarning`, `LexPhase`/`ParsePhase`/`SemanticPhase`. **Scope is the module, not the package:** a probe (2026-08-10) confirmed a variant in a separate module may reuse a name already declared in `diagnostic.fab` and still check green — each `.fab` module is a namespace, cross-module access is `module.name` qualified. Each migrate-* goal needs internal de-duplication within its own module only; no global coordination. (Untested: name ambiguity at *use* sites when two imported modules each export the same variant/class name — expected to surface with the first cross-module consumer.)
